...
function qMatrixAns = RMRC_test(robot) 
% 1.1) Set parameters for the simulation
    % robot = TM5;        % Load robot model
    t = 10;             % Total time (s)
    ...
    qMatrixTM5 = zeros(steps,6);       % Array for joint anglesR
    ...
    qMatrixTM5(1,:) = robot.model.ikcon(T,q0);                                            % Solve joint angles to achieve first waypoint
    ...
    for i = 1:steps-1
        ...
        T = robot.model.fkine(qMatrixTM5(i,:)).T;                                           % Get forward transformation at current joint state
        ...
        J = robot.model.jacob0(qMatrixTM5(i,:));                                   % Get Jacobian at current joint state
        m(i) = sqrt(det(J*J'));
        ...
        for j = 1:6                                                             % Loop through joints 1 to 6
            if qMatrixTM5(i,j) + deltaT*qdot(i,j) < robot.model.qlim(j,1)                     % If next joint angle is lower than joint limit...
                qdot(i,j) = 0; % Stop the motor
            elseif qMatrixTM5(i,j) + deltaT*qdot(i,j) > robot.model.qlim(j,2)                 % If next joint angle is greater than joint limit ...
                qdot(i,j) = 0; % Stop the motor
            end
        end
        qMatrixTM5(i+1,:) = qMatrixTM5(i,:) + deltaT*qdot(i,:);
        qMatrixAns(i, :) = qMatrixTM5(i, :);
